<script setup>
    ...
    async function fetchInitData(){
        await apiClient.get('dosen/find', {
            params: {
                id_prodi: sessionStore.session.mhswa.id_prodi
            }
        }).then(resp => {
        ...
        await apiClient
            .get('jadwal', {
                params: {
                    ...
                    id_kelas: sessionStore.session.mhswa.id_kelas,
                    role: 'Mahasiswa'
                }
            })
            .then(resp => {
    ...
    function openTanggal(id){
        router.push(`/dashboard/mahasiswa/absensi/${id}`)
    }
        ...
        <div class="flex items-center justify-end gap-3">
            <div class="flex p-2 bg-darkbrown rounded-lg items-center gap-2 max-w-[50%] text-sm">
                ...
                <select class="bg-transparent text-white outline-none *:bg-softcream *:text-black w-full" v-model="kelas">
                    <option selected value="0">Semua dosen</option>
